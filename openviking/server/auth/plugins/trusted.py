...
_TRUSTED_REGISTER_IDENTITY_HEADER = "X-OpenViking-Register-Identity"
...
def _should_register_identity(request: Request) -> bool:
    raw = request.headers.get(_TRUSTED_REGISTER_IDENTITY_HEADER)
    if raw is None:
        return False
    ...
    if value == "false":
        return False
    raise InvalidArgumentError(f"{_TRUSTED_REGISTER_IDENTITY_HEADER} must be true or false.")
...
class TrustedAuthPlugin(AuthPlugin):
    ...
    async def resolve_identity(
        ...
        *,
        ...
        if (
            not is_admin_path
            and _should_register_identity(request)
    ...
    async def initialize(self, app, service, config) -> None:
        ...
        self._known_max_size = config.trusted_identity_known_max_size
        self._flush_task = asyncio.create_task(self._flush_loop())

    def _queue_trusted_identity(self, account_id: str, user_id: str) -> None:
        identity = (account_id, user_id)
        if identity in self._known or user_id in self._in_flight.get(account_id, set()):
            return
    ...
    def _mark_known(self, identities: dict[str, set[str]]) -> None:
        for account_id, user_ids in identities.items():
            for user_id in user_ids:
                identity = (account_id, user_id)
                self._known.pop(identity, None)
                self._known[identity] = None
        while len(self._known) > self._known_max_size:
