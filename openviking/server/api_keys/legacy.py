...
class LegacyAPIKeyManager:
    ...
    async def create_account(
        ...
        try:
            ...
            await self._save_users_json(account_id, {admin_user_id: user_info})
    ...
    def get_users(
        ...
        for user_id, user_info in account.users.items():
            ...
            if user_info.get("identity_source"):
                user_data["identity_source"] = user_info["identity_source"]
            if expose_key:
    ...
    async def _save_users_json(
        ...
        *,
        ...
        """Merge targeted user mutations into the latest locked registry snapshot."""
        path = USERS_PATH_TEMPLATE.format(account_id=account_id)
        try:
