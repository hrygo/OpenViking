...
async def test_ensure_trusted_identities_creates_keyless_users_once(
    ...
    result = await manager.ensure_trusted_identities(
    ...
    assert manager._legacy._accounts[acme].users["alice"]["identity_source"] == "trusted"
    ...
    assert manager.get_users(acme, expose_key=True) == [
        ...
        {"user_id": "bob", "role": "user", "identity_source": "trusted"},
    ]
